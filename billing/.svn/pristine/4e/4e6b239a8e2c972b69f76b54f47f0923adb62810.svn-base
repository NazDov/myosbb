...
import ua.ssmt.softserveinc.com.lvklegals.webservice.applogic.datalayer.queries.select.SearchAgreementContrAgentQuery;
 ...
 */
public class SearchAgreementToComboBox extends AbstractSelectOperation<ComboBoxItem> {

    private AgreementCondition condition;

    public SearchAgreementToComboBox(AgreementCondition condition){
    ...
    public List<ComboBoxItem> doOperation(Connection connection)   {
        List<Agreement> agreements = new SearchAgreementContrAgentQuery(condition).execute(connection);
