 ...
 */
public class CreateCorrectionQuery extends AbstractModifyQuery<Correction> {
    @Override
    protected String getSQLQuery() {
        return "INSERT INTO [dbo].[DocCorrection]\n" +
                "           ([DateCorrection]\n" +
                "           ,[DocNumber1]\n" +
                "           ,[Contragent1ID]\n" +
                "           ,[DocNumber2]\n" +
    ...
    protected void fillPreparedStatement(PreparedStatement st, Correction value) throws SQLException {
        ...
        if (value.getAgreementDepOne().getId() != null){
            st.setLong(12,value.getAgreementDepOne().getId());
        } else st.setNull(12, 0);
        if (value.getAgreementDepTwo().getId() != null){
